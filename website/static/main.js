...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function addJoint() {
        ...
        // Check if the user has the required subscription status
        if (!userSubscriptionStatus && jointCount >= maxJointsWithoutPermission) {
            showGenericModal(
    ...
    // retrieve user permissions asynchronoously from the flask app
    // Retrieve user subscription status asynchronously from the Flask app
...
function showGenericModal(message, pageUrl, linkTitle) {
    ...
    if (modalBody) {
        ...
        $('#genericModal').modal('show'); // Requires jQuery and Bootstrap's JS
    } else {
